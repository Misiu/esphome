...
void ESP32RMTOneWireBus::dump_config() {
  ESP_LOGCONFIG(TAG, "ESP32 RMT 1-wire bus:");
  LOG_PIN("  Pin: ", this->pin_);
  this->dump_devices_(TAG);
}

// ---------------------------------------------------------------------------
// reset_int(): send a 500 µs LOW reset pulse and detect the presence pulse
// Returns: 1 = device(s) present, 0 = no device, -1 = signal error
// ---------------------------------------------------------------------------
int ESP32RMTOneWireBus::reset_int() {
...
// ---------------------------------------------------------------------------
// write8() / write64() — pure TX, no interrupt involvement
// ---------------------------------------------------------------------------
void ESP32RMTOneWireBus::write8(uint8_t val) {
  rmt_transmit(this->tx_channel_, this->tx_bytes_encoder_, &val, 1, &TX_CONFIG);
  rmt_tx_wait_all_done(this->tx_channel_, 50);
}

void ESP32RMTOneWireBus::write64(uint64_t val) {
...
// ---------------------------------------------------------------------------
uint8_t ESP32RMTOneWireBus::read8() {
  uint8_t tx_buf = 0xFF;
  uint8_t result = 0;
...
uint64_t ESP32RMTOneWireBus::read64() {
  uint8_t tx_buf[8];
...
// ---------------------------------------------------------------------------
// Single-bit helpers used by the ROM search algorithm
// ---------------------------------------------------------------------------
bool ESP32RMTOneWireBus::read_bit_() {
...
uint64_t ESP32RMTOneWireBus::search_int() {
